...
# DEFAULT SETTING
    def __init__(self, iface, parent=None):
        ...
        self.pb_add_all_lod1.clicked.connect(self.addall_lod1)
        self.pb_add_all_lod2.clicked.connect(self.addall_lod2)
        self.pb_add_all_lod3.clicked.connect(self.addall_lod3)
        self.pb_add_all_lod4.clicked.connect(self.addall_lod4)        
        self.pb_add_per_building.clicked.connect(self.addall_per_building)
...
# Tab 2 - Database Connection and Adding Layer 
    ...
    def filllist(self):
        ...
        self.file_path = os.path.join(pluginPath, 'db_list.json')
        with open(self.file_path, "r", encoding = 'utf-8') as file:
            self.db_liste = json.load(file)
...
# Tab 8 -Test
    ...
    def addall_lod1(self):
     try :
       ...
       def showMessage(title, mesage):
                QMessageBox.information(None, title, mesage)
       showMessage("Success",dbname + " load sucesfully")

     except:
       def showMessage(title, mesage):
                QMessageBox.information(None, title, mesage)
       showMessage("Failed To Load", "Fill The Parameter first!")

       
    # Add All LOD 2
    def addall_lod2(self):
     try :
       # Tabel Exterior - Interior 
       dbname = self.dbname.text()
       user = self.user.text()
       pw = self.pw.text()
       host = self.host.text()
       ...
       curr.execute("CREATE TABLE them_surface_lod2 AS (SELECT geometry, objectclass_id, building_id FROM surface_geometry INNER JOIN thematic_surface ON root_id = lod2_multi_surface_id)")
    ...
    def addall_lod3(self):
     try :
       ...
       uri.setConnection(str(host), "5432", str(dbname), str(user), str(pw))
       uri.setDataSource("citydb", tablename, "geometry", "objectclass_id = 34")
       addgeom=QgsVectorLayer (uri .uri(False), dbname +" Wall_lod3", "postgres")
       QgsProject.instance().addMapLayer(addgeom)
       ...
       curr.execute("DROP TABLE them_surface_lod3,them_surface_opening_lod3")
    ...
    def addall_lod4(self):
     try :
       ...
       curr.execute("DROP TABLE  them_surface_lod4,them_surface_room_lod4,them_surface_furniture_lod4,them_surface_installation_lod4")
    ...
    def addall_per_building(self):
       ...
       curr.execute("SELECT COUNT(*) FROM building")
       
       a = str(curr.execute("SELECT COUNT(*) FROM building"))
       
       self.filllist()
       self.label_20.setText("There are "+a+" buildings")             